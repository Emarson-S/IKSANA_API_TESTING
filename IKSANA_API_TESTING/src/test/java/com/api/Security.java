    ...
    public void LoginWithOTP() throws IOException {
        try {
            ...
            APIResponse response1 = request().get(DevOrigin + "login-with-otp",
                    RequestOptions.create().setHeader("Authorization", auth).setHeader("channel", "Mobile")
                            .setHeader("Application", "Iksana-Base"));
            int statusCode1 = response1.status();
            System.out.println("statusCode1--->" + statusCode1);
            ...
            // <------------ Login_TC_12, Login_TC_13, Login_TC_14, Login_TC_15,
            // Login_TC_16, Login_TC_17, Login_TC_18 ------------->

            for (int i = 1; i < 8; i++) {
                String value3 = toReadDataFromExcel("Registration", 0, i) + ":"
                        + toReadDataFromExcel("Registration", 1, i);
                String auth3 = "Basic " + encryption(value3);
                ;
                APIResponse response3 = request().get(DevOrigin + "login-with-otp",
                        RequestOptions.create().setHeader("Authorization", auth3).setHeader("channel", "Mobile")
                                .setHeader("Application", "Iksana-Base"));
                int statusCode3 = response3.status();
                System.out.println("statusCode3--->" + statusCode3);
        ...
        try {
            ...
            APIResponse response1 = request().get(DevOrigin + "login",
                    RequestOptions.create().setHeader("Authorization", auth)
                            .setHeader("channel", toReadDataFromExcel("Users", 7, 1))
                            .setHeader("Application", "Iksana-Base"));
            int statusCode1 = response1.status();
            System.out.println("statusCode1--->" + statusCode1);
            ...
            // <------------ Login_TC_22,Login_TC_23, Login_TC_24, Login_TC_25, Login_TC_26,
            // Login_TC_27, Login_TC_28, Login_TC_29, Login_TC_30, ------------->
            ...
            for (int i = 1; i < 10; i++) {
                ...
                APIResponse response2 = request().get(DevOrigin + "login",
                        RequestOptions.create().setHeader("Authorization", auth1)
                                .setHeader("channel", toReadDataFromExcel("Users", 7, i))
                                .setHeader("Application", "Iksana-Base"));
                int statusCode2 = response2.status();
                System.out.println("statusCode2--->" + statusCode2);
                ...
                if (statusCode2 == 200) {
                    ...
                    if (d1.equals("8888")) {
                        ...
                        APIResponse response = postRequestWithoutToken("session-verification",
                                toReadDataFromExcel("Users", 7, i), data);
                        int StatusCode1 = response.status();
                        System.out.println("statusCode1--->" + StatusCode1);
                        try {
                            Assert.assertEquals(StatusCode1, 200);
    ...
    public void Logout() throws IOException {
        for (int i = 1; i < 10; i++) {
            APIResponse response1 = request().put(DevOrigin + "signout",
                    RequestOptions.create().setHeader("Authorization", "Bearer " + toReadDataFromExcel("Users", 2, i))
                            .setHeader("Application", "Iksana-Base"));
            int StatusCode1 = response1.status();
            System.out.println("statusCode1--->" + StatusCode1);
